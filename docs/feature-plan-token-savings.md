...
and gets back the top-k relevant chunks (a few hundred tokens each) with `file:line` citations it
can follow precisely. Generation can additionally be offloaded to the local model entirely, so
...
- `search_codebase(query, repo?, k?, token_budget?)` — returns compact ranked chunks.
  ...
  files — returns the most relevant code snippets with file:line citations."* Steering text is
